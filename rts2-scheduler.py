...
def load_horizon_v2(file_path: str, location: EarthLocation) -> interp1d:
    """Load horizon data from a file and create an interpolation function."""
    az = []
    alt = []
    ha = []
...
def load_horizon_v1(file_path: str, location: EarthLocation) -> interp1d:
    """Load horizon data from a file and create an interpolation function."""
    az = []
    alt = []
    is_azalt = False
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            if is_azalt:
    #            print(f"horizon: azalt coords: {x} {y}")
                az.append(x)
    #            if y<30: y=30
                alt.append(y)
            else:
                alt_y, az_x = hadec_to_altaz(x * 15 * u.deg, y * u.deg, location)
    #            print(f"horizon: hadec coords: {x} {y} -> {az_x} {alt_y}")
                
                az.append(az_x)
                alt.append(alt_y)

    # Ensure circular condition
    if az[0] != 360:
        az.append(360)
        alt.append(alt[0])
    ...
    return interp1d(az, alt, kind='linear', fill_value='extrapolate')
...
def prepare_sun_positions_v1(start_time, end_time, slice_size, resources):
    """Pre-compute sun positions for all time slices."""
    time_diff = (end_time - start_time).total_seconds()
    n_slices = int(time_diff / slice_size)
    ...
    for resource_name, resource_info in resources.items():
        ...
        # Store both altitude and below_horizon mask
        sun_positions[resource_name] = {
            'altitudes': sun_altaz.alt.deg,
            'is_night': sun_altaz.alt < -15*u.deg
        }
...
def old_fetch_requests(conn, slice_size: int) -> List[Request]:
    """
    Fetch observation requests from the database and create Request objects.

    Args:
        conn: Database connection object.
        slice_size (int): Size of time slices in seconds.

    Returns:
        List[Request]: List of Request objects.
    """
    requests = []
    #median_durations = Request.calculate_median_durations(conn)
    #default_duration = 1800  # 30 minutes as default if no historical data

    query = """
        SELECT tar_id, tar_ra, tar_dec, tar_priority, tar_name, 
               tar_comment, tar_bonus, tar_bonus_time, tar_next_observable, 
               tar_info, interruptible, tar_pm_ra, tar_pm_dec, tar_telescope_mode
        FROM targets
        WHERE tar_enabled = TRUE AND tar_id BETWEEN 1000 AND 49999
    """
    ...
    try:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            cur.execute(query)
            for row in cur:
                request = Request.from_db_row(row, median_durations, default_duration, slice_size)
                if request:
...
def calculate_visibility_batch(targets, resources, start_time, end_time, slice_size_seconds, horizon_func, avoid_meridian_flip=False):
    """Batch process visibility calculations for all targets."""
    time_diff = (end_time - start_time).total_seconds() / 3600
    time_range = Time(start_time) + np.linspace(0, time_diff, int(time_diff * 60))*u.hour
    
    # Pre-calculate sun positions for all resources
    sun_positions = {}
    for resource_name, resource_info in resources.items():
        observatory = resource_info['earth_location']
        altaz_frame = AltAz(obstime=time_range, location=observatory)
        sun_altaz = get_sun(time_range).transform_to(altaz_frame)
        sun_positions[resource_name] = sun_altaz.alt < -15*u.deg
    ...
    for target in targets:
        ...
        for resource_name, resource_info in resources.items():
            observatory = resource_info['earth_location']
            altaz_frame = AltAz(obstime=time_range, location=observatory)
            sun_low = sun_positions[resource_name]
            
            if target is not None:
                ra, dec = target.tar_ra, target.tar_dec
                ...
                if avoid_meridian_flip:
                    lst = time_range.sidereal_time('apparent', observatory.lon)
                    hour_angle = lst - target_coord.ra
                    hour_angle = hour_angle.wrap_at(180 * u.deg)
                    meridian_flip = np.diff(np.sign(hour_angle.deg)) != 0
                    meridian_flip = np.insert(meridian_flip, 0, False)
                    is_visible = is_visible & ~meridian_flip
            else:
                is_visible = sun_low
            ...
            visible_intervals = []
            start = None
            for t, visible in zip(time_range, is_visible):
                if visible and start is None:
                    start = t
                elif not visible and start is not None:
                    aligned_start = start.datetime.replace(microsecond=0)
                    aligned_start += timedelta(seconds=(slice_size_seconds - aligned_start.timestamp() % slice_size_seconds) % slice_size_seconds)
                    
                    aligned_end = t.datetime.replace(microsecond=0)
                    aligned_end -= timedelta(seconds=aligned_end.timestamp() % slice_size_seconds)
                    
                    if aligned_end > aligned_start:
                        visible_intervals.append((aligned_start, aligned_end))
                    start = None
            ...
            if start is not None:
                ...
                aligned_end = time_range[-1].datetime.replace(microsecond=0)
                aligned_end -= timedelta(seconds=aligned_end.timestamp() % slice_size_seconds)
        ...
        results[target.id] = visibility_intervals
    
    return results

def calculate_visibility(target, resources, start_time, end_time, slice_size_seconds, horizon_func, avoid_meridian_flip=False):
    time_diff = (end_time - start_time).total_seconds() / 3600
    
    def create_time_range():
        return Time(start_time) + np.linspace(0, time_diff, int(time_diff * 60))*u.hour
    ...
    for resource_name, resource_info in resources.items():
        ...
        def setup_frame():
            return AltAz(obstime=time_range, location=observatory)
        
        altaz_frame = setup_frame()
        
        def calculate_sun():
            sun_altaz = get_sun(time_range).transform_to(altaz_frame)
            return sun_altaz.alt < -15*u.deg
        
        sun_low = calculate_sun()

        if target is not None:
            def calculate_target_position():
                ra, dec = target[1], target[2]
                target_coord = SkyCoord(ra=ra*u.degree, dec=dec*u.degree, frame='icrs')
                target_altaz = target_coord.transform_to(altaz_frame)
                return target_coord, target_altaz
            
            target_coord, target_altaz = calculate_target_position()
            
            def check_horizon():
                horizon_alt = horizon_func(target_altaz.az.degree)
                return target_altaz.alt.degree > horizon_alt
            
            target_high = check_horizon()
            ...
            def compute_visibility():
                ...
                if avoid_meridian_flip:
                    ...
                    meridian_flip = np.diff(np.sign(hour_angle.deg)) != 0
                    meridian_flip = np.insert(meridian_flip, 0, False)
                    return is_visible & ~meridian_flip
                return is_visible
        ...
        visibility_intervals[resource_name] = create_windows()
    
    return visibility_intervals

def old_calculate_visibility(target, resources, start_time, end_time, slice_size_seconds, horizon_func, avoid_meridian_flip=False):
    time_diff = (end_time - start_time).total_seconds() / 3600
    time_range = Time(start_time) + np.linspace(0, time_diff, int(time_diff * 60))*u.hour
    ...
    for resource_name, resource_info in resources.items():
        ...
        sun_altaz = get_sun(time_range).transform_to(altaz_frame)
        # lets not do any reservations for times when the Sun is higher than -15deg
        sun_low = sun_altaz.alt < -15*u.deg
        ...
        if target is not None:
            ...
            # Apply horizon function
            horizon_alt = horizon_func(target_altaz.az.degree)
            target_high = target_altaz.alt.degree > horizon_alt and target_altaz.alt > 20
            ...
            if avoid_meridian_flip:
                ...
                # Check for meridian flip (hour angle crossing 0)
                meridian_flip = np.diff(np.sign(hour_angle.deg)) != 0
                
                # Extend meridian_flip array to match is_visible
                meridian_flip = np.insert(meridian_flip, 0, False)

                # Update visibility to avoid meridian flip
                is_visible = is_visible & ~meridian_flip
        else:
            is_visible = sun_low
        ...
        for t, visible in zip(time_range, is_visible):
            if visible and start is None:
                ...
                # Align the start and end times to the slice size
                aligned_start = start.datetime.replace(microsecond=0)
                aligned_start += timedelta(seconds=(slice_size_seconds - aligned_start.timestamp() % slice_size_seconds) % slice_size_seconds)
...
# Function to calculate visibility windows for a target
def calculate_visibility_no_meridian_avoidance(target, resources, start_time, end_time, slice_size_seconds, horizon_func):
    # Calculate the time difference in hours
    time_diff = (end_time - start_time).total_seconds() / 3600

    # Create a time range from start_time to end_time
    time_range = Time(start_time) + np.linspace(0, time_diff, int(time_diff * 60))*u.hour
    ...
    for resource_name, resource_info in resources.items():
        ...
        sun_altaz = get_sun(time_range).transform_to(altaz_frame)
        sun_low = sun_altaz.alt < -10*u.deg

        if target is not None:
            ra, dec = target[1], target[2]
            target_coord = SkyCoord(ra=ra*u.degree, dec=dec*u.degree)
            target_altaz = target_coord.transform_to(altaz_frame)
            target_high = target_altaz.alt.degree > horizon_func(target_altaz.az.degree)
            is_visible = sun_low & target_high
        else:
            is_visible = sun_low
...
def get_targets_v2(conn, slice_size, resources, start_time, end_time, horizon_func):
    """
    Fetch targets and compute their visibility windows efficiently.
    """
    # First get all targets from database
    ...
    # Pre-compute sun positions once for all slice centers
    time_diff = (end_time - start_time).total_seconds()
    n_slices = int(time_diff / slice_size)
    slice_centers = [
        start_time + timedelta(seconds=slice_size * i + slice_size/2)
...
def get_targets_old(conn, slice_size, resources, start_time, end_time, horizon_func):
    # Fetch targets
    ...
    with timing("for request in requests"):
        for request in requests:
            # Calculate visibility windows for the request
            visibility_windows = calculate_visibility_batch(
                (request.id, request.tar_ra, request.tar_dec),
                resources, start_time, end_time, slice_size, horizon_func,
                avoid_meridian_flip=True
            )
            
            # Create possible_windows_dict
            possible_windows_dict = {
                resource_name: Intervals(visibility_windows[resource_name])
                for resource_name in resources.keys()
            }
            
            # Remove intervals smaller than the duration for each resource
            for resource_name, intervals in possible_windows_dict.items():
                intervals.remove_intervals_smaller_than(request.duration)
            
            # Update the request's possible windows
            request.possible_windows_dict = possible_windows_dict
